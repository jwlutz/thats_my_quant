...
**Source**: 13F filing deltas between quarters
**Metrics**: 
...
**Rationale**: Institutional money movements are the strongest sentiment signal
...
#### 3. Public Sentiment (Optional, Flag-Gated)
**Reddit**: Official PRAW API (requires credentials)
**X (Twitter)**: Official API v2 (requires credentials)
**Default**: DISABLED (requires explicit env config)

**Rationale**: Valuable signal but requires external credentials and rate limiting
...
```python
def calculate_sentiment_score(ticker: str, window_days: int = 7) -> dict:
    """
    Calculate comprehensive sentiment score.
    
    Components:
    1. Institutional score: 13F delta z-score (weight: 40%)
    2. News score: Time-weighted FinBERT scores (weight: 40%) 
    3. Public score: Social sentiment average (weight: 20%, if enabled)
    ...
    Returns:
    {
        ...
        'components': {
            'institutional': {'score': float, 'weight': float, 'data_points': int},
            'news': {'score': float, 'weight': float, 'data_points': int},
            'public': {'score': float, 'weight': float, 'data_points': int}
        },
        'catalysts': [{'type': str, 'description': str, 'impact': float}],
        'relevance_filter': {'total_items': int, 'relevant_items': int}
    }
    """
```
